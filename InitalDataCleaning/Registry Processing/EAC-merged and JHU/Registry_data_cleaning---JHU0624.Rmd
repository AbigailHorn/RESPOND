---
title: "Registry cleaning report - JHU 0624"
author: "Weijia Hu"
date: "`r Sys.time()`"
output:
  html_document:
...
load("vjhuid2_20210615.Rda")
d_jhu <- vjhuid2
load("vregall20210601.Rda")
d_regall <- vregall

colnames(d_jhu) <- "rid"

d <- merge(d_jhu, d_regall,by="rid")
```
...
```{r respondid}
  ...
  respondid <- trimws(d[,"rid"])
 
  new.d <- data.frame(new.d, respondid)
  new.d <- apply_labels(new.d, respondid = "respond_id")
  new.d.1 <- data.frame(new.d.1, respondid)
...
st_css() #IMPORTANT!
  ...
  cssitespecificfactor8[which(cssitespecificfactor8=="010")]<-"10"
  cssitespecificfactor8[which(cssitespecificfactor8=="998")]<-"NA"
  cssitespecificfactor8[which(cssitespecificfactor8=="999")]<-"10"

  for (i in 1:length(cssitespecificfactor8)){
    if (!is.na(cssitespecificfactor8[i])){
      if (nchar(cssitespecificfactor8[i])==3){
        cssitespecificfactor8[i]<-substring(cssitespecificfactor8[i],3)
      }
    }
  }
  
  cssitespecificfactor8<-as.factor(cssitespecificfactor8)
  cssitespecificfactor8 <- ordered(cssitespecificfactor8, c("4","5","6","7","8","9","10"))
...
st_css() #IMPORTANT!
  derivedajcc7t <-d[,"derivedajcc7t"]
  derivedajcc7t[which(derivedajcc7t=="999")]<-"NA"
  derivedajcc7t<-as.factor(derivedajcc7t)
...
```{r }
write.table(new.d, file="output_JHU_0624.txt", sep="\t")
```
