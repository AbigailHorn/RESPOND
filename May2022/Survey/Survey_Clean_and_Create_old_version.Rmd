---
title: "Survey data cleaning report - Original version"
author: "Weijia Hu"
date: "`r Sys.time()`"
output:
  html_document:
...
require(zoo) # for date conversion
require(expss) # for labels
require(DBI) # for Database connectivity
require(summarytools) # for summarizing variables
require(tidyverse)
require(plyr)
require(dplyr)
require(knitr) # for creating tables
require(questionr)
...
```{r input survey data}
load("../Data/vsurvall20220421.Rda")
d <- vsurvall
```
...
- FH_SecDeg=0;
  + if B1Da=2 then FH_SecDeg+1; maternal grandfather
  + if B1Ea=2 then FH_SecDeg+1; paternal grandfather
...
```{r output file}
write.table(new.d, file="survey_data_cleaned_20211209.txt", sep="\t", quote=F, row.names=F)
